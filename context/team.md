# Team — iFundCities

## Engineering (report to Scot)

### Dynamics Team
- **Suyog** — Dynamics team lead, CRM development and coordination
- **Aditi Salunkhe** — Power Apps/Dataverse architecture
- **Dhanesh Vakte** — CRM development
- **Dinesh** — Servicing automation, daily reports

### Cloud-Native Team (contractors)
- **Nathan Toups** — Architect/SRE/DevOps/platform. 28hr/wk. Ex-Flyr colleague. Pragmatic counterbalance to Scot. Intent to hire FT. Working on infrastructure, data platform, Gruntweave.
- **Aleksandre (Alex) Javakhishvili** — Full-stack .NET Core dev (Toptal). 20hr/wk. Portal development.
- **Innocent (Inno) Musanzikwa** — Data engineer (Toptal). Full-time. Azure Databricks (EDH/ALH). Onboarded March 5, 2026.

### India Operations
- ~52 employees focused on operational work (document processing, etc.)
- Sushanth — loan processing, post-closing
- Nikhil Gupta — QC, compliance

## Key Business Stakeholders

- **Bryan Ziegenfuse** — CEO/President. Portal and data app stakeholder. Runs Level 10 meetings.
- **Ryan** — Business stakeholder. Portal, data, AI tooling strategy.
- **Harry Murray** — Bridge underwriting lead. AI credit memo vision, document vault concept.
- **Brittany Croce** — Servicing/asset management. CRM pain points, BSI data needs.
- **Max Herlan** — Draws/inspections. AI pilot champion (budget translation, inspection grading). Sitewire integration.
- **Allison (Ali) Ladley** — Late payment collections, servicing automation.
- **Matthew Volk** — DSCR underwriting. Loan locking is #1 pain point.
- **Jack Davis** — Capital markets. DSCR pricing, rate sheet ingestion.
- **Mike Mendenhall** — AM/processing. CRM development process.
- **Tina Elmer** — Marketing.
- **Cody Creason** — Draws team. Slow draw workflows.
- **Erik Eichinger** — Business operations.

## External Vendors

- **Sitewire** — Inspection platform, API integration upcoming (~65% of inspections)
- **BSI** — Loan servicing system
- **JourneyTEAM** — Microsoft licensing partner
- **Toptal** — Staffing agency for Alex and Inno
